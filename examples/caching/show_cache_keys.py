...
def show_cache_keys():
    ...
    concept_key = get_cache_key(
        201826,  # Type 2 diabetes concept ID
        method_name="src.ohdsi_webapi.services.vocabulary.VocabularyService.get_concept"
    )
    print(f"Individual concept (201826):")
    ...
    print(f"Basic concept search ('diabetes'):")
    ...
    print(f"Filtered concept search ('diabetes', domain='Condition', standard='S'):")
    print(f"  Key: {search_filtered_key}")
    print()
    
    # 4. List all domains
    domains_key = get_cache_key(
        method_name="src.ohdsi_webapi.services.vocabulary.VocabularyService.list_domains"
    )
    print(f"List all domains:")
    print(f"  Key: {domains_key}")
    print()
    
    # 5. List all concept sets
    concept_sets_key = get_cache_key(
        method_name="src.ohdsi_webapi.services.concept_sets.ConceptSetService.list"
    )
    print(f"List all concept sets:")
    print(f"  Key: {concept_sets_key}")
    print()
    
    # 6. Concept descendants
    descendants_key = get_cache_key(
        201826,
        method_name="src.ohdsi_webapi.services.vocabulary.VocabularyService.descendants"
    )
    print(f"Concept descendants (201826):")
    ...
    print(f"Same search with force_refresh (key identical):")
    ...
    order1_key = get_cache_key(
        "arg1", "arg2",
        method_name="test.method",
        param1="value1",
        param2="value2"
    )
    order2_key = get_cache_key(
        "arg2", "arg1",  # Different order
        method_name="test.method", 
        param1="value1",
        param2="value2"
    )
    print(f"Parameter order sensitivity:")
    ...
    kw1_key = get_cache_key(
        "query",
        method_name="test.method",
        param_a="value_a",
        param_b="value_b"
    )
    kw2_key = get_cache_key(
        "query", 
        method_name="test.method",
        param_b="value_b",  # Different order
        param_a="value_a"
    )
    print(f"Keyword parameter order (sorted internally):")
