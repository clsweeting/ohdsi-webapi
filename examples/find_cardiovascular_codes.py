...
async def find_cardiovascular_codes():
    ...
    broad_concepts = [
        c for c in condition_concepts 
        if len(c.conceptName.split()) <= 4 and 'disease' in c.conceptName.lower()
    ]
...
async def create_cardiovascular_concept_sets():
    ...
    combined_expression = {
        "id": 0,
        "name": "Major Cardiovascular Conditions",
        "expression": {
            "items": []
        }
    }
...
async def test_cardiovascular_cohort():
    ...
    try:
        ...
        approaches = [
            {
                "name": "All CVD patients",
                "filters": []
            },
            {
                "name": "Male CVD patients 40+", 
                "filters": [
        ...
        for approach in approaches:
            try:
                results = await client.cohorts.build_incremental_cohort(
                    ...
                    base_name=f"CVD Study - {approach['name']}",
                    concept_sets=[cvd_cs],
