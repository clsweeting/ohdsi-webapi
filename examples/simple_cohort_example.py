...
async def example_incremental_cohort():
    ...
    diabetes_concepts = client.cohorts.create_concept_set(
    ...
    cohort_base = CohortDefinition(
        name=f"All diabetes patients{UNIQUE_SUFFIX}",
    ...
    cohort_male = CohortDefinition(
        name=f"Male diabetes patients{UNIQUE_SUFFIX}", 
    ...
    cohort_age = CohortDefinition(
        name=f"Male diabetes patients 40+{UNIQUE_SUFFIX}",
        expression=age_expression
    )
    ...
    final_expression = client.cohorts.add_time_window_filter(
        age_expression,
        concept_set_id=0,  # diabetes concept set
        days_before=730,   # 2 years
        filter_name="Diabetes in last 2 years"
    )
    
    cohort_final = CohortDefinition(
        name=f"Male diabetes patients 40+ (last 2 years){UNIQUE_SUFFIX}",
...
async def example_automated():
    ...
    results = await client.cohorts.build_incremental_cohort(
        ...
        base_name=f"Males 40+ Diabetes{UNIQUE_SUFFIX}",
