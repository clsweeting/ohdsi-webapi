...
class WebApiClient:
    def __init__(self, base_url: str, *, auth: Optional[AuthStrategy] = None, timeout: float = 30.0, verify: bool | str = True):
        self._http = HttpExecutor(base_url.rstrip('/'), timeout=timeout, auth_headers_cb=(auth.auth_headers if auth else None), verify=verify)
