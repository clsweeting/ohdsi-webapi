...
class TestServiceCacheIntegration:
    ...
    def test_vocabulary_search_caching(self):
        ...
        results3 = self.client.vocabulary.search("diabetes", domain_id="Condition")
        assert call_count == 2  # Additional API call
    ...
    def test_multiple_services_cache_isolation(self):
        ...
        respx.get(f"{self.base_url}/vocabulary/concept/123").mock(
            return_value=httpx.Response(200, json=concept_data)
        )
        respx.get(f"{self.base_url}/conceptset/123").mock(
            return_value=httpx.Response(200, json=concept_set_data)
        )
